...
export const AIStateProvider = ({ children, stateManager, client }: PropsWithChildren<{
  stateManager?: StateManager
  client?: IAIClient
}>) => {
  const stateManagerInternal = useMemo(() => {
    if (stateManager && client) {
      console.warn('AIStateProvider: Both stateManager and client provided. Using stateManager.');
    }
    ...
    throw new Error('AIStateProvider requires either a stateManager or a client');
  }, [stateManager, client]);

  const getState = useCallback(() => stateManagerInternal, [stateManagerInternal]);
  const internalSubscribe = useCallback((event: Events, callback: () => void) => {
